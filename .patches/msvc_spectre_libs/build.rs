...
fn add_spectre_link_search() {
    ...
    if spectre_libs.exists() {
        ...
        // Degrade gracefully instead of panicking: the build can proceed
        // without the spectre-mitigated CRT libs (they are only needed for
        // /Qspectre builds). This is a local patch to allow building on
        // machines without the VS Spectre component installed.
        println!("cargo:warning=No spectre-mitigated libs were found; proceeding without them.");
    }
}
