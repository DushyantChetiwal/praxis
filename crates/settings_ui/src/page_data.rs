...
fn ai_page(cx: &App) -> SettingsPage {
    ...
    fn agent_configuration_section(_cx: &App) -> Box<[SettingsPageItem]> {
        ...
        items.extend([
            SettingsPageItem::SettingItem(SettingItem {
                title: "Loop Guard",
                description: "Interrupt the model when it starts repeating itself, instead of letting a stuck model fill the context window.",
                field: Box::new(SettingField {
                    organization_override: None,
                    json_path: Some("agent.loop_guard.enabled"),
                    pick: |settings_content| {
                        settings_content
                            .agent
                            .as_ref()?
        ...
        items.extend([
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "When auto compaction runs. A percentage string like \"70%\" is measured against the context window. A positive integer is the number of used tokens to compact after. A negative integer is the number of tokens remaining in the context window before compacting.",
                field: Box::new(SettingField {
                    organization_override: None,
