...
In native-agent Plan mode, an MCP tool is offered only when its MCP annotations explicitly set `readOnlyHint: true`. A read-only tool with `openWorldHint: true` is treated as an external read; other explicitly read-only tools are treated as local reads. Missing or false read-only hints fail closed and remain unavailable until the thread enters Build mode. The annotation is a declaration by the MCP server, so enable only servers you trust to describe their tools honestly.
